...
from model.model_utils import Residual_Linear_Layer, knn, local_cov
...
class Decoder_Block(nn.Module):
    ...
    def build_grid(self, batch_size, sample_grid, device, edge_only=False):
        if self.shape == "plane":
            ...
            points = (
                self.sphere if not sample_grid else self.sample_spherical(self.num_points_sqrt * self.num_points_sqrt)
            )
...
class Decoder_Linear(Decoder_Block):
    ...
    def decode(self, x, grid):
        ...
        if self.std is None:
            ...
            return {"reconstruction": folding_result2.transpose(1, 2), "std": std}  # (batch_size, num_points ,3)
    ...
    def forward(self, x, pts_orig, sample_grid=False, edge_only=False):
        if self.point_encoding:
            ...
            if (
                self.grid is None or x.shape[0] != self.grid.shape[0]
            ):  # doesn't exist or sample or batch_size has changed
